...
const StakeholderForm: React.FC<StakeholderFormProps> = ({
  ...
  const { register, handleSubmit, reset } = useForm<CreateStakeholderData>({
    defaultValues: initialData ? { ...initialData, projectId } : { projectId },
  });
  ...
  const onSubmit = async (data: CreateStakeholderData) => {
    if (initialData) {
      await updateStakeholder(initialData.id, data);
    } else {
